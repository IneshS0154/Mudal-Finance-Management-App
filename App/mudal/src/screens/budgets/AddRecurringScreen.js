...
// ── Preset recurring categories ─────────────────────────────────────────
const RECURRING_CATEGORIES = [
  { _id: 'rc_01', name: 'Business Income', icon: 'freelance', color: '#4CAF50', type: 'income' },
  { _id: 'rc_02', name: 'Salary',          icon: 'salary', color: '#03A9F4', type: 'income' },
  { _id: 'rc_other_inc', name: 'Other',    icon: 'other', color: '#9E9E9E', type: 'income' },
  
  { _id: 'rc_03', name: 'Housing',         icon: 'rent', color: '#FF9800', type: 'expense' },
  { _id: 'rc_04', name: 'Water Bills',     icon: 'water', color: '#2196F3', type: 'expense' },
  { _id: 'rc_05', name: 'Electricity',     icon: 'electricity', color: '#FFC107', type: 'expense' },
  { _id: 'rc_06', name: 'Internet',        icon: 'internet', color: '#00BCD4', type: 'expense' },
  { _id: 'rc_07', name: 'Phone Bills',     icon: 'phone', color: '#9C27B0', type: 'expense' },
  { _id: 'rc_08', name: 'Gym',             icon: 'fitness', color: '#E91E63', type: 'expense' },
  { _id: 'rc_09', name: 'Loan',            icon: 'other', color: '#607D8B', type: 'expense' },
  { _id: 'rc_10', name: 'Insurance',       icon: 'insurance', color: '#8BC34A', type: 'expense' },
  { _id: 'rc_11', name: 'Subscriptions',   icon: 'subscriptions', color: '#FF5722', type: 'expense' },
  { _id: 'rc_other_exp', name: 'Other',    icon: 'other', color: '#9E9E9E', type: 'expense' },
];
...
const AddRecurringScreen = ({ navigation, route }) => {
  ...
  const [customCategoryName, setCustomCategoryName] = useState(
    edit?.category && !RECURRING_CATEGORIES.find(c => c.name === edit.category.name) ? edit.category.name : ''
  );
  ...
  useEffect(() => {
    if (!edit) {
      ...
      // If the edited category name doesn't exist in our presets, it must be a custom one
      const isCustom = !RECURRING_CATEGORIES.find(c => c.name === edit.category.name);
      
      setSelectedCategory(isCustom ? RECURRING_CATEGORIES.find(c => c._id === (edit.type === 'income' ? 'rc_other_inc' : 'rc_other_exp')) : edit.category);
      setCustomCategoryName(isCustom ? edit.category.name : '');
  ...
  const handleSave = async () => {
    ...
    if (selectedCategory.name === 'Other' && !customCategoryName.trim()) {
      Alert.alert('Missing Category Name', 'Enter a name for the custom category'); return;
    }
    ...
    const payload = {
      ...
      category: {
        ...selectedCategory,
        name: selectedCategory.name === 'Other' ? customCategoryName.trim() : selectedCategory.name
      },
