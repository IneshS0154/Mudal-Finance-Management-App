...
const getAllRecurring = async (req, res) => {
  try {
    ...
    const items = await Recurring.find({ user: req.user.id }).sort({ nextDueDate: 1 });
    res.json(items);
  } catch (err) {
...
const getRecurringById = async (req, res) => {
  try {
    ...
    if (!item) return res.status(404).json({ message: 'Recurring transaction not found or not authorized' });
    res.json(item);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
